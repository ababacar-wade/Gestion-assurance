  ...
  <div class="col-lg-4">
    <div class="card-sunu card mb-4">
      <div class="card-body text-center p-4">
        ...
        <span class="badge {{ $client->is_active?'bg-success':'bg-danger' }}" style="border-radius:20px;">{{ $client->is_active?'Actif':'Inactif' }}</span>
      </div>
    </div>
    <div class="card-sunu card">
      <div class="card-body">
        <h6 style="font-family:'Plus Jakarta Sans',sans-serif;font-weight:700;margin-bottom:1rem;">Coordonnées</h6>
        @foreach([['📞','Téléphone',$client->telephone??'—'],['🏠','Adresse',$client->adresse??'—'],['💼','Profession',$client->profession??'—']] as [$ic,$lb,$vl])
        <div style="display:flex;gap:.75rem;padding:.6rem 0;border-bottom:1px solid var(--gray-border);">
            <span>{{ $ic }}</span><div><div style="font-size:.7rem;color:var(--text-muted);">{{ $lb }}</div><div style="font-weight:600;font-size:.875rem;">{{ $vl }}</div></div>
        </div>
        @endforeach
      </div>
    </div>
      ...
      <div class="card-body">
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;padding:.75rem;background:var(--gray-bg);border-radius:12px;margin-bottom:.5rem;">
            ...
            <div class="d-flex align-items-center gap-2">
                ...
                <a href="{{ route('agent.contrats.show', $c) }}" style="font-size:.78rem;color:var(--orange);font-weight:600;text-decoration:none;">Voir →</a>
            </div>
        </div>
        @empty <p style="color:var(--text-muted);font-size:.875rem;">Aucun contrat</p>
        @endforelse
      </div>
    </div>
    <div class="card-sunu card">
      <div class="card-body">
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;padding:.75rem;background:var(--gray-bg);border-radius:12px;margin-bottom:.5rem;">
            <div><div style="font-weight:700;font-size:.875rem;">{{ $s->numero_sinistre }}</div><div style="font-size:.78rem;color:var(--text-muted);">{{ $s->date_sinistre?->format('d/m/Y') }}</div></div>
            <span class="badge bg-warning text-dark" style="font-size:.72rem;">{{ $s->statut }}</span>
        </div>
        @empty <p style="color:var(--text-muted);font-size:.875rem;">Aucun sinistre</p>
        @endforelse
      </div>
    </div>
  </div>
</div>
<div class="mt-3"><a href="{{ route('agent.clients.index') }}" style="color:var(--text-muted);font-size:.875rem;font-weight:600;text-decoration:none;">← Retour à mes clients</a></div>
@endsection